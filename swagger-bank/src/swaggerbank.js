...
class API {
  ...
  getAllObjectDefinitions() {
    if (this.apiObject.definitions == null) {
      throw new Error(
        "Please provide any object references inside a 'definitions' section inside your swagger spec"
      );
    }
  ...
  constructTemplateForRef(refSchema, refName) {
    ...
    if (refSchema.type === "array") {
      ...
      if (refSchema.items.type === "object") {
        const objectTemplate = this.constructTemplateForRef(
          refSchema.items,
          refName
        );
      ...
      // ARRAY OF OBJECT REFERENCES
        ...
        if (objectSchema === undefined) {
          console.error(
            "Your object schema is undefined. This most likely means you are referencing an object that doesn't have a definition"
          );
          process.exit(-1);
        }
      ...
      if (propertyInfo.$ref === undefined) {
        ...
        if (propertyType === "array") {
          ...
          // ARRAY OF OBJECTS DEFINED VIA $REF
            ...
            // Extract object name from $ref tag
            const objectName = propertyInfo.items.$ref.replace(
              "#/definitions/",
              ""
            );
          ...
          // ARRAY OF NOTHING
            ...
            propertyInfo.items === "" ||
            propertyInfo.items === undefined ||
            JSON.stringify(propertyInfo.items, null, 2) === "{}"
          ) {
            console.warn(
              colors.yellow(
                `WARNING:  Encountered an array without a type for its items. Generating an empty array instead for ${property}`
              )
            );
          ...
          // SOMETHING BAD
            ...
            throw new Error(
              colors.red(
                `PARSE ERROR: You most likely defined an array of different types of objects. Don\'t do that. Here\'s the faulty spec: ${JSON.stringify(
                  propertyInfo,
                  null,
                  2
                )}`
  ...
  getParametersObject(uriVerbObject) {
    ...
    try {
      ...
      if (e instanceof TypeError) {
        throw new TypeError(
          `ERROR [functionCalled = getParametersObject] : Could not find a parameterObject for uri: ${uri} verb: ${verb} error: ${
            e.message
          }`
        );
      } else {
        throw new Error(
          `ERROR [functionCalled = getParametersObject] : ${e.message} `
        );
      }
    }
  ...
  async resolveSubSpec(path) {
    return Swagger.resolveSubtree(this.apiObject, [].concat(path)).then(
      result => result.spec
    );
  }
   ...
   */
  async getAllResponsesForApi() {
    // directly extract the definitions section of the swagger spec
    const apiDefinitions = await this.resolveSubSpec("definitions");
    ...
    for (const route in paths) {
      ...
      for (const verb in paths[route]) {
        ...
        for (const status in paths[route][verb].responses) {
          ...
          const newUrl =
            this.getApiBasePath() +
            urlManager.RegExifyURL(route, routeParametersObject);
          ...
          if (responseObject.$ref !== undefined) {
            const referencedResponseName = responseObject.$ref.replace(
              "#/responses/",
              ""
            );
          ...
          const responseSchema = await this.resolveSubSpec([
            "paths",
            route,
            verb,
            "responses",
          ...
          if (typeof responseObject["x-mb-auto-stub-id"] === "undefined") {
            responseObject["x-mb-auto-stub-id"] = shortId.generate();
            this.addStubId(
              `paths:${route}:${verb}:responses:${statusCode}`,
              responseObject["x-mb-auto-stub-id"]
            );
          }
          ...
          if (process.env.PROP_GEN === "example") {
            if (responseObject.examples == null) {
              throw new Error(
                `ERROR: process.env.PROP_GEN was set to example but the response for ${route} ${verb} ${statusCode} did NOT contain an examples section`
              );
            }
            ...
            completeResponse.res.responseHeaders["x-mb-auto-stub-id"] =
              responseObject["x-mb-auto-stub-id"];
          ...
          // STATIC OR RANDOM MODE - This gets handled inside of responseManager
            ...
            const referencedTemplate = this.constructTemplateForRef(
              responseSchema,
              "property"
            );
            ...
            if (populatedTemplate.property !== undefined)
              populatedTemplate = populatedTemplate.property;
            const completeResponse = responseManager.constructCompleteResponse({
              uri: newUrl,
              method: method,
              statusCode: statusCode,
  ...
  addStubId(pathStr, id) {
    this.generatedStubIds.push({ path: pathStr, stubId: id });
  }

  /**
   *
   */
  async setupMountebankImposter(imposterPortNumber) {
    ...
    const allResponses = await this.getAllResponsesForApi();
    allResponses.forEach(function(element) {
      Imposter.addRoute(element);
    });
    return mountebankHelper.startMbServer(2525).then(() => {
      return Imposter.postToMountebank()
        .then(responseBody => {
          console.log(
            `[SWAGGER-BANK] SUCCESS: Your Imposter is now listening!! Use localhost:${imposterPortNumber}${this.getApiBasePath()} to start testing your swagger routes`
          );
          return responseBody;
        })
        .catch(error => {

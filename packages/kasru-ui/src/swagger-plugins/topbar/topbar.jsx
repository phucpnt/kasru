...
class OAS3GeneratorMessage extends React.PureComponent {
  render() {
    ...
    return (
      <div onClick={this.props.showModal} className="long-menu-message">
...
export default class Topbar extends React.Component {
  ...
  importFromFile = () => {
    ...
    fileReader.onload = fileLoadedEvent => {
      ...
      this.props.specActions.updateSpec(
        YAML.safeDump(YAML.safeLoad(textFromFileLoaded))
      );
      this.hideModal();
    };
  ...
  startMB() {
    let specName = this.props.specSelectors.specName();
    let editorContent = this.props.specSelectors.specStr();
    let stubContent = this.props.specSelectors.stubStr();
    this.props.specActions.runMBServerSession(specName, {
  ...
  render() {
    let {
      getComponent,
      specSelectors: { isOAS3, isMBLoading, mockUrn, specName }
    } = this.props;
      ...
      <div>
          ...
          <div className="topbar-wrapper">
            ...
            <div style={{ marginLeft: "auto" }}>
              {mockUrn() === undefined && (
                <Button
                  color={"red"}
                  disabled={specName() === undefined}
                  onClick={this.startMB.bind(this)}
                >
                  Run mock server
                </Button>
              )}
              {mockUrn() !== undefined && (
                <Button color={"blue"} onClick={this.startMB.bind(this)}>
                  {isMBLoading() ? "Loading..." : "Save and run MB"}
                </Button>
              )}
            </div>
            }
        ...
        <Modal className="swagger-ui modal" ref="generatorModal">
          <div className="modal-message">
            <p>
              Code generation for OAS3 is currently work in progress. The
              available languages is smaller than the for OAS/Swagger 2.0 and is
              constantly being updated.
            </p>
            <p>
              If you encounter issues with the existing languages, please file a
              ticket at&nbsp;
              <a
                href="https://github.com/swagger-api/swagger-codegen-generators"
                target={"_blank"}
              >
                swagger-codegen-generators
              </a>. Also, as this project highly depends on community
              contributions - please consider helping us migrate the templates
              for other languages. Details can be found at the same repository.
            </p>
            <p>Thanks for helping us improve this feature.</p>
          </div>
          <div className="right">
            <button
              className="btn"
              onClick={this.hideModal.bind(null, "generatorModal")}
            >
              Close
            </button>
          </div>
